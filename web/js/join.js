...
$('#okagree').on('click',function () {
    if (!$('#agree1').is(':checked'))
        ...
        location.href = '/join/checkme1.do';
}); // 모두 동의함
...
$('#checkbtn').on('click',function () {
   ...
   else if (!$('#female').is(':checked') && !$('#male').is(':checked'))
       alert('성별을 선택하세요.');
   else
       location.href = '/join/verifyme1.do?name=' + $('#name').val();
}); // 실명확인하기
...
$('#requestbtn').on('click',function () {
    if ($('#ph1').val()=='국번' || $('#ph2').val()=='' || $('#ph3').val()=='')
        alert('전화번호를 정확히 입력하세요.');
    else
        location.href = '/join/joinme1.do?name=' + $('#name').val();
}); // 인증번호 요청
